...
setwd("C:/Users/djor8013/OneDrive - The University of Sydney (Staff)/En Health/air-health-sws-r-targets-main")
```
...
$$AN = \sum(1 - e^{(1 - \beta\triangle_{i,j})}) \times Expected_{i,j}$$

Where $Expected_i_j$ is the death count estimated by applying the mortality rate in age-group $i$ by age-specific population counts within SA2 $j$, $β=log(RR)/10$ and $\triangle X_i_j$ is the change in annual PM2.5 concentration from baseline concentrations to counterfactuals concentrations in SA2 $j$. Baseline concentrations were estimated as the population-weighted PM2.5 levels for each SA2 by year.   
